...
bool g_bPreGravityEnabled = true;
bool g_bGravityEnabled = false;
...
void initTweakBar() {
	...
	TwType TW_GRAVITY_ENABLED = TwDefineEnumFromString("Gravity State", g_pSimulator->getGravityState());
	TwAddVarRW(g_pDUC->g_pTweakBar, "Gravity State", TW_GRAVITY_ENABLED, &g_bGravityEnabled, "");
...
void CALLBACK OnFrameMove( double dTime, float fElapsedTime, void* pUserContext )
{
	...
	if (g_bPreGravityEnabled != g_bGravityEnabled) {
		if (g_pDUC->g_pTweakBar != nullptr) {
			TwDeleteBar(g_pDUC->g_pTweakBar);
			g_pDUC->g_pTweakBar = nullptr;
	}
		initTweakBar();
		g_pSimulator->notifyGravityChanged(g_bGravityEnabled);

		g_pSimulator->initUI(g_pDUC);
		g_bPreGravityEnabled = g_bGravityEnabled;
}
